...
def goban_para_str(g: 'list[list[str]]') -> str:
    ...
    for i in range(rows, 0, -1):
        ...
        field += str(i) + " "
        for j in range(cols):
            field += pedra_para_str(obtem_pedra(g,
                                    cria_intersecao(caracter("A", j), i))) + " "
        field += " " if i < 10 else ""
        field += str(i) + "\n"
...
def turno_jogador(g: 'list[list[str]]', p: str, l: 'list[list[str]]') -> bool:
    ...
    while not (
        ...
        turno_atual = input(
            "Escreva uma intersecao ou 'P' para passar [{}]:".format(pedra_para_str(p)))
...
def jogo_para_str(g: 'list[list[str]]', t: 'list[int, int]') -> str:
