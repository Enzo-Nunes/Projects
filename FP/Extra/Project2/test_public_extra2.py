...
class TestMarcosOrdenaIntersecoes:
    def test_1(self):  # Linhas
        ...
        answer = ("A1", "A2", "A3", "A4", "A5", "A6",
                  "A7", "A8", "A9", "A10", "A18")
        assert ordena_intersecoes(i1) == tuple(
            str_para_intersecao(x) for x in answer)
    ...
    def test_3(self):  # Linhas e Colunas
        i1 = ("G1", "A1", "K1", "B1", "C1", "E1", "F1", "H1", "D1", "I1", "J1",
              "A18", "A10", "A1", "A2", "A3", "A4", "A5", "A6", "A7", "A8", "A9")
        i1 = tuple(str_para_intersecao(x) for x in i1)
        answer = ("A1", "A1", "B1", "C1", "D1", "E1", "F1", "G1", "H1", "I1", "J1",
                  "K1", "A2", "A3", "A4", "A5", "A6", "A7", "A8", "A9", "A10", "A18")
        assert ordena_intersecoes(i1) == tuple(
            str_para_intersecao(x) for x in answer)
...
class TestMarcosObtemCadeia:
    ...
    def test_3(self):
        ip = ("A1", "A2", "A3", "B3", "B4", "C4",
              "D4", "D3", "D2", "C2", "C1", "B1")
        ip = tuple(str_para_intersecao(x) for x in ip)
        g = cria_goban(9, (), ip)
        answer = ("A1", "B1", "C1", "A2", "C2", "D2",
                  "A3", "B3", "D3", "B4", "C4", "D4")
...
class TestMarcosnColocaPedra:
    ...
    def test_6(self):
        ...
        answer = ()
        for i in ib:
            answer += (obtem_pedra(g, i)
                       if eh_pedra_jogador(obtem_pedra(g, i))else ())
        assert answer == ()
...
class TestMarcosObtemTerritorios:
    def test_1(self):
        ib = ("B1", "A2", "B2", "A4", "B4", "C4", "D4", "D3", "D2", "D1", "F1", "F2", "F3", "F4", "F5", "F6", "E6", "D6", "C6", "B6",
              "A6", "A8", "B8", "C8", "D8", "E8", "F8", "G8", "H8", "H7", "H6", "H5", "H4", "H3", "H2", "H1", "I9", "G7", "E5", "C3")
        ib = tuple(str_para_intersecao(x) for x in ib)
        g = cria_goban(9, ib, ())
        answer = (('A1',), ('C1', 'C2'), ('E1', 'E2', 'E3', 'E4'), ('G1', 'G2', 'G3', 'G4', 'G5', 'G6'), ('I1', 'I2', 'I3', 'I4', 'I5', 'I6', 'I7',
                  'I8'), ('A3', 'B3'), ('A5', 'B5', 'C5', 'D5'), ('A7', 'B7', 'C7', 'D7', 'E7', 'F7'), ('A9', 'B9', 'C9', 'D9', 'E9', 'F9', 'G9', 'H9'))
        answer = tuple(tuple(str_para_intersecao(j)
                       for j in i) for i in answer)
        assert obtem_territorios(g) == answer

    def test_2(self):
        ip = ("B1", "A2", "B2", "A4", "B4", "C4", "D4", "D3", "D2", "D1", "F1")
        ib = ("F2", "F3", "F4", "F5", "F6", "E6", "D6", "C6", "B6", "A6", "A8", "B8", "C8", "D8",
              "E8", "F8", "G8", "H8", "H7", "H6", "H5", "H4", "H3", "H2", "H1", "I9", "G7", "E5", "C3")
        ip = tuple(str_para_intersecao(x) for x in ip)
        ib = tuple(str_para_intersecao(x) for x in ib)
...
class TestMarcosObtemAdjacentesDiferentes:
    def test_1(self):
        ...
        answer = ('E1', 'G1', 'E2', 'G2', 'E3', 'G3', 'E4', 'G4', 'A5',
                  'B5', 'C5', 'D5', 'G5', 'G6', 'A7', 'B7', 'C7', 'D7', 'E7', 'F7')
        answer = tuple(str_para_intersecao(x) for x in answer)
...
class TestMarcosEhJogadaLegal:
    ...
    def test_1(self):  # Intersecao invalida
        ...
        assert not eh_jogada_legal(
            g, cria_intersecao("K", 1), cria_pedra_branca(), l)
    ...
    def test_2(self):  # Intersecao ocupada
        ...
        l = cria_copia_goban(g)
        assert not eh_jogada_legal(
            g, cria_intersecao("A", 1), cria_pedra_branca(), l)

    def test_3(self):  # Regra do KO
        g = cria_goban_vazio(9)
        copy = cria_copia_goban(g)
        coloca_pedra(copy, cria_intersecao("A", 1), cria_pedra_branca())
        assert not eh_jogada_legal(g, cria_intersecao(
            "A", 1), cria_pedra_branca(), copy)
    ...
    def test_4(self):  # Regra do Suicidio
        ...
        l = cria_copia_goban(g)
        assert not eh_jogada_legal(
            g, cria_intersecao("A", 1), cria_pedra_preta(), l)
    ...
    def test_5(self):  # Sem liberdade na pedra mas com liberdade na cadeia
        ...
        g = cria_goban(9, ib, ())
        l = cria_copia_goban(g)
        assert eh_jogada_legal(g, cria_intersecao(
            "A", 1), cria_pedra_branca(), l)
...
class TestMarcosTurnoJogador:
    def test_1(self):  # Regra do Suicídio | Intersecao invalida | Intersecao ocupada
        ...
        g = cria_goban(9, ib, ip)
        l = cria_copia_goban(g)
        turno_jogador_offline(g, cria_pedra_branca(),
                              l, "A1\nA2\nB2\nA3\nA4\n")
