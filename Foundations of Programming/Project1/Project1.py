...
def limpa_texto(t):
    """ string -> string
        Limpa  o  texto, t, de caracteres brancos, resultando numa string em que
        as palavras estão separadas por um espaço."""
...
def corta_texto(s, n):
    """ string * inteiro -> string * string
        Corta  a  entrada,  s,  segundo um dado tamanho de coluna, n. Retorna um
        tuplo com dois elementos que correspondem à entrada cortada."""
    ...
    if n < len(first_word):
        ...
        return (s, "")
    # Corta a entrada no espaço anterior à posição "n" na entrada.
    else:
        space_before_n = s[n::-1].find(" ")
...
def insere_espacos(s, n):
    """ string * inteiro -> string
        Insere  espaços  entre as palavras da entrada, s, até esta ter o tamanho
        de coluna desejado, n."""
    ...
    if n < len(s):
        return s
    # Destribui os espaços necessários entre as palavras.
    spaces = s.count(" ")
    length = len(s) - spaces
    spaces_per_space = (n - length)//spaces
    res = s.replace(" ", spaces_per_space * " ")
    # Determina se devem ser adicionados espaços extra ou não para completar as linhas.
    if (n - length) % spaces == 0:
        return res
    else:
...
def justifica_texto(t, n):
    ...
    # Divide a entrada em colunas com o tamanho desejado.
    clean = corta_texto(clean, n)
    cut = ()
    while len(clean[1]) > n:
        cut = cut + (clean[0], )
        clean = corta_texto(clean[1], n)
    cut = cut + (clean[0], clean[1], )
    # Insere espaços em cada linha até estas terem o tamanho de coluna desejado.
    res = ()
    for i in range(len(cut[:-1])):
        elem = insere_espacos(cut[i], n)
        res = res + (elem, )
    # Adiciona espaços no fim da útima linha se necessário.
    if cut[-1] == "":
        return res
...
def calcula_quocientes(d, n):
    """ dicionário * inteiro -> dicionário
        Calcula  os  quocientes de cada partido indicado em d, concorrentes a um
        círculo com n deputados, segundo o Método de Hondt."""
...
def atribui_mandatos(d, n):
    """ dicionário * inteiro -> lista
        Retorna  uma  lista  com  os  deputados  apurados  a  cada  mandato,  n,
        por ordem, a partir dos votos de cada partido em d."""
    ...
    for i in range(n):
        ...
        # Retorna as keys correspondentes a cada partido que possuam o valor máximo de votos.
        keys        = key_lookup_quocientes(quocients, max_value)
        match       = len(keys.copy())
        # Adiciona os partidos ao resultado, por ordem.
        while keys != [] and len(res) != n:
            count   = [res.count(x) for x in keys]
            res    += [keys[count.index(min(count))]]
            keys.remove(keys[count.index(min(count))])
        # Em "values", troca os valores adicionados ao resultado pelos seus próximos valores.
...
def next_value(v, first_v):
    """ inteiro * inteiro -> inteiro
        Retorna  o  quociente  seguinte  ao  quociente  "v" de um dado partido a
        partir do quociente inicial, "first_v"."""
...
def obtem_partidos(d):
    """ dicionário -> lista
        Retorna  os partidos por ordem alfabética participantes nas eleições dos
        círculos de um dado território."""
...
def obtem_resultado_eleicoes(d):
    """ dicionário -> lista
        Determina,  pelo  método de Hondt, o número de deputados eleitos de cada
        partido nos círculos de um dado território."""
    ...
    mandatos    = []
    # Cria a lista ordenada com o número de mandatos atribuidos a cada partido.
    ...
    # Cria a lista com os resultados de cada partido nas eleições de um dado território.
    for i in range(len(partidos)):                              
        for x in votos:
            if partidos[i] in x.keys():
                total_votos[i] += x[partidos[i]]
        # Ordena a lista de acordo com o número de deputados eleitos e votos obtidos.
        res += [(partidos[i], mandatos[i], total_votos[i])]
...
def verifica_convergencia(m, c, x, e):
    """ tuplo * tuplo * tuplo * real -> booleano
        Verifica  se  a  estimativa "x" para a solução do sistema está próxima o
        suficiente do valor verdadeiro pela precisão "e"."""
...
def retira_zeros_diagonal(m, c):
    """ tuplo * tuplo -> tuplo * tuplo
        Permuta  as linhas  da matriz  "m" de  modo a que não  haja zeros na sua
        diagonal principal."""
...
def eh_diagonal_dominante(m):
    """ tuplo -> booleano
        Verifica  se  a  matriz "m" é diagonalmente dominante, ou seja, se, para
        cada linha, o valor absoluto da diagonal é superior à soma dos restantes
        valores absolutos da linha."""
...
def resolve_sistema(m, c, e):
    """ tuplo * tuplo * real -> tuplo
        Determina  a  estimativa  "x",  de  precisão "e", pelo método de Jacobi,
        para  a  solução do sistema de equações representado pela matriz "m" e o
        vetor de contantes "c"."""
    ...
    #Aplica a formula do Método de Jacobi a cada linha.
